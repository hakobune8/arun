...
name: %s
description: Minimal chart generated by AgentOS for the implementation-heavy scrum workflow.
type: application
version: 0.1.0
appVersion: "0.1.0"
`, projectName)
...
`
	...
	if commandAvailable("helm") {
		...
		if err := runShell(ctx, root, fmt.Sprintf("helm template agentos-validation %q >/dev/null", chartDir)); err != nil {
			return "", err
		}
	}
