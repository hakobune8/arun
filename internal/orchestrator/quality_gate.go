...
const helmValidationCommand = `sh -c 'set -e; chart=$(find . -path "*/Chart.yaml" -not -path "./.git/*" -print -quit); test -n "$chart"; dir=$(dirname "$chart"); test -f "$dir/values.yaml"; test -d "$dir/templates"; grep -R "^kind: Deployment" "$dir/templates" >/dev/null; grep -R "^kind: Service" "$dir/templates" >/dev/null; if command -v helm >/dev/null 2>&1; then rendered=$(mktemp); helm lint "$dir"; helm template arun-validation "$dir" >"$rendered"; grep -q "^kind: Deployment" "$rendered"; grep -q "^kind: Service" "$rendered"; fi'`
