...
const servedFrontendRuntimeValidationCommand = `sh -c 'if [ -f server/go.mod ] && [ -f server/main.go ] && [ -f client/index.html ] && command -v go >/dev/null 2>&1 && command -v curl >/dev/null 2>&1; then tmp=$(mktemp -d); port=$((20000 + ($$ % 20000))); pid=""; cleanup(){ if [ -n "$pid" ]; then kill "$pid" >/dev/null 2>&1 || true; wait "$pid" 2>/dev/null || true; fi; rm -rf "$tmp"; }; trap cleanup EXIT; (cd server && go build -o "$tmp/app" .); (cd server && PORT="$port" "$tmp/app" >"$tmp/server.log" 2>&1) & pid=$!; sleep 1; curl -fsS "http://127.0.0.1:$port/" >/dev/null; for ref in $(grep -Eo "(href|src)=\"[^\"]+\\.(css|js)([#?][^\"]*)?\"" client/index.html | sed -E "s/^(href|src)=\"([^\"#?]+).*/\\2/" | sed -E "s#^\\./##"); do case "$ref" in http://*|https://*|//*) continue;; esac; clean=$(printf "%s" "$ref" | sed -E "s/[#?].*$//"); clean=${clean#/}; curl -fsS "http://127.0.0.1:$port/$clean" >/dev/null || { echo "server runtime does not serve /$clean from client/index.html"; exit 1; }; done; fi'`
...
const qaValidationCommand = `sh -c 'if [ -f package.json ]; then PM=npm; [ -f pnpm-lock.yaml ] && PM=pnpm; [ -f yarn.lock ] && PM=yarn; [ -f bun.lockb ] && PM=bun; if command -v node >/dev/null 2>&1 && command -v "$PM" >/dev/null 2>&1; then ran=0; for script in lint typecheck test build; do if node -e "const p=require(\"./package.json\"); process.exit(p.scripts&&p.scripts[process.argv[1]]?0:1)" "$script"; then ran=1; case "$PM:$script" in yarn:*) yarn "$script";; pnpm:*) pnpm "$script";; bun:*) bun run "$script";; *) npm run "$script";; esac; fi; done; test "$ran" = 1 || test -f docs/smoke-test.md || test -f README.md; else test -f docs/smoke-test.md || test -f docs/testing.md || test -f README.md; fi; fi; moddir=; [ -f server/go.mod ] && moddir=server; [ -z "$moddir" ] && [ -f go.mod ] && moddir=.; if [ -n "$moddir" ]; then if command -v go >/dev/null 2>&1; then (cd "$moddir" && pkgs=$(go list ./...) && test -n "$pkgs" && go test ./...); else main=server/main.go; [ "$moddir" = "." ] && main=main.go; test -f "$main" && grep -q "/healthz" "$main"; fi; elif [ ! -f package.json ]; then test -f docs/testing.md || test -f docs/smoke-test.md || test -f README.md; fi'`
...
func qualityGateForSubtask(subtask *Subtask) *QualityGate {
	...
	case "docker":
		return &QualityGate{
			...
			ValidationCommands: []string{frontendValidationCommand, servedFrontendRuntimeValidationCommand, dockerValidationCommand},
			ContentChecks: []QualityContentCheck{{
				File:     "Dockerfile",
